# =============================================================================
# Packer Template: Windows Server 2025 for Proxmox VE
# =============================================================================
#
# This template creates a Windows Server 2025 base image on Proxmox with:
...
#
# NOTE: Using BIOS (SeaBIOS) instead of UEFI to fix DVD boot timeout issues
#
# Prerequisites:
#   1. Windows Server 2025 ISO uploaded to Proxmox storage
#   2. VirtIO drivers ISO uploaded (virtio-win.iso)
#   3. Proxmox API token with appropriate permissions
#
# Usage:
#   cd packer/windows-server-2025-proxmox
#   packer init .
...
variable "proxmox_api_url" {
  type        = string
  description = "Proxmox API URL (e.g., https://192.168.20.22:8006/api2/json)"
}
...
variable "ws2025_iso_file" {
  type        = string
  description = "Windows Server 2025 ISO filename on Proxmox storage"
  default     = "en-us_windows_server_2025_updated_oct_2025_x64_dvd_6c0c5aa8.iso"
}
...
variable "winrm_host" {
  type        = string
  description = "Static IP for WinRM during build"
  default     = "192.168.80.99"
}
...
source "proxmox-iso" "ws2025" {
  ...
  vm_id                = var.vm_id
  vm_name              = var.vm_name
  template_description = "Windows Server 2025 Standard - Built ${local.timestamp} - Hybrid Lab"

  # Hardware - Using BIOS (SeaBIOS) instead of UEFI to fix DVD boot timeout
  cores      = var.vm_cores
  ...
  # Boot order - CD-ROM (ide2) first for Windows installation
  boot = "order=ide2;scsi0;net0"

  # NOTE: Removed efi_config and tpm_config blocks for BIOS mode
  # TPM 2.0 is not required for WS2025 with BIOS
  ...
  # ISO Configuration
  iso_file = "${var.proxmox_iso_storage}:iso/${var.ws2025_iso_file}"
  ...
  # Boot Configuration - BIOS boots faster than UEFI
  # Send multiple key presses to catch the "Press any key to boot from CD" prompt
  boot_wait    = "3s"
  boot_command = [
